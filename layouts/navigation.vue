<template>
    <div class="fixed top-0 left-0 w-full" style="z-index: 1000 !important;">
        <section class="w-full bg-color-main flex align-middle justify-center  py-1  px-0 lg:px-2 xl:px-2 2xl:px-2  md:px-4 lg:px-8 xl:px-8 2xl:px-4">
        <div class="w-11/12 flex flex-row justify-between align-middle hidden lg:contents xl:contents 2xl:contents">
            ...
            <div class="w-full inline-flex gap-2 align-middle justify-end">
                ...
                <!-- <UButton :ui="{ rounded: 'rounded-full' }" variant="ghost"  size="md"
                    ...
                    <UButton :ui="{ rounded: 'rounded-full' }" variant="solid" size="md"
                    class="bg-white text-green-500 text-sm hover:text-green-300 hover:bg-green-500"  v-if="authStore.isAuthenticated" @click="logout()">{{$t('nav_sign_out')}}</UButton>
            </div>
        </div>
        <div class="w-full flex flex-row justify-between lg:hidden xl:hidden 2xl:hidden">
            ...
            <div class="w-full flex flex-row justify-end align-middle gap-0">
                <!-- <UButton :ui="{ rounded: 'rounded-full' }" variant="ghost"  size="sm"
                    ...
                    <register  v-if="!authStore.isAuthenticated" />
                    <UButton :ui="{ rounded: 'rounded-full',  }" variant="solid" size="sm"
                    class="w-[130px] h-[45px] block bg-white text-green-500 text-sm hover:text-green-300 hover:bg-green-500"  v-if="authStore.isAuthenticated" click="logout()">{{$t('nav_sign_out')}}</UButton>
    ...
    <section  v-if="humbergerMenuOpened"  class="lg:hidden xl:hidden 2xl:hidden bg-white w-full shadow-lg 	shadow-gray-300">
            <div class="">
                <UButton :ui="{ rounded: 'rounded-none',  }" variant="ghost" size="sm"
                    class="w-full h-ful bg-white py-2  text-gray-900  text-xl hover:bg-green-200" >{{$t('nav_learn_with_cend')}}</UButton>
                <UButton :ui="{ rounded: 'rounded-none',  }" variant="ghost" size="sm"
                    class="w-full h-ful bg-white  text-gray-900  text-xl hover:bg-green-200" >{{$t('nav_tutorat')}}</UButton>
                <UButton :ui="{ rounded: 'rounded-none',  }" @click="aboutMenuClicked()" variant="ghost" size="sm" :trailing-icon="aboutMenuOpened ? 'i-heroicons-chevron-up' : 'i-heroicons-chevron-down'"
                    class="w-full h-ful bg-white  text-gray-900  text-xl hover:bg-green-200"  >{{$t('nav_about_us')}}</UButton>
                    ...
                    <UButton :ui="{ rounded: 'rounded-none',  }" variant="ghost" size="sm"
                    class="w-full h-ful bg-white  text-gray-900  text-xl hover:bg-green-200"  >{{$t('nav_help')}}</UButton>
            </div>
        </section>
    </div>
</template>
