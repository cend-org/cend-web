                    ...
                    <div class="p-2">
                        <div class=" flex flex-col gap-1">
                            <div class="flex flex-row justify-content-between w-full gap-1" v-for="renumeration in renumerations">
                                <div v-if="renumeration.name == 'first_try'" class="w-full p-2 border-2 border-gray-400">{{$t(renumeration.name)}}</div>
                                <div v-if="renumeration.name != 'first_try'" class="w-full p-2 border-2 border-gray-400">{{renumeration.name}}</div>
                                <div  class="w-full p-2 border-2 border-gray-400 text-center" >
                                    ...
                                    <div v-if="renumeration.editable"  class=" w-full h-full inline-flex gap-2 align-items-center">
                                            <input autocomplete="off" class="w-full text-center rounded-lg bg-green-100" ref="moneyInput" type="text" name="currency" v-model="state.renumeration" @input="onInput" @focus="onFocus" @keydown="onKeydown">
                                    </div>
                                </div>
                            </div>
                        </div>
...
async function onSubmit() {
    ...
    //     navigateTo("/authentications/register/tutor/renumeration");
