...
async function onSubmit() {
    ...
    loadingStore.show();
    GqlNewTutor({ email: state.email }).then(response => {
        LocalStorageSetItem(environment.auth_token, response.NewTutor?.T);
        navigateTo("/authentications/register/tutor/password");
    }, error => {
        loadingStore.hide();
        toast.add(
            {
