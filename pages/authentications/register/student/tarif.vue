            ...
            <form action="" class="w-full flex flex-col gap-2 pt-3">
                <UForm :state="state" class="space-y-6" @submit="onSubmit">
                    <div class="py-3 text-lg poppins-medium">
                        {{ $t('register_student_tarif_a') }} <span class="text-color-main poppins-bold">{{ tarif }}</span>, {{ $t('register_student_tarif_b') }} <span
                            class="text-color-main poppins-bold">{{ subjects }}</span> {{ $t('register_student_tarif_c') }} <span
                    ...
                    <div class="py-2 text-md poppins-regular text-gray-500">
                        {{ $t('register_student_tarif_f') }} 
                         <span
                            class="poppins-medium text-color-main">{{ dispo_date }}</span>   {{ $t('at') }} <span
