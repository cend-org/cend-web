        ...
        <div class="p-2 w-full lg:w-[25rem] xl:w-[25rem] 2xl:w-[25rem] flex flex-col gap-2">
            <form action="" class="w-full flex flex-col gap-2 pt-3">
                ...
                <UForm :state="state" class="space-y-4" @submit="onSubmit">
                    ...
                    <UFormGroup :label="$t('register_lang')" name="lang">
                        <lang />
                    </UFormGroup>
...
function getAcademicLevels() {
    GqlAcademicLevels().then(response => {
        ...
        selectionMultipleStore.list = response.AcademicLevels as Array<never>;
        navigateTo('/authentications/register/teacher/academic-level');
    }, error => {
        loadingStore.hide();
        toast.add(
            {
