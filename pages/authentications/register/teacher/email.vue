...
async function onSubmit() {
    ...
    loadingStore.show();
    GqlNewProfessor({ email: state.email }).then(response => {
        LocalStorageSetItem(environment.auth_token, response.NewProfessor?.T);
        navigateTo("/authentications/register/teacher/password");
    }, error => {
        loadingStore.hide();
        toast.add(
            {
