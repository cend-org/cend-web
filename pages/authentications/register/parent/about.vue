            ...
            <form action="" class="w-full flex flex-col gap-2 pt-3">
                ...
                <UForm :state="state" class="space-y-4" @submit="onSubmit">
                    ...
                    <UFormGroup :label="$t('register_birthdate')" name="date-of-birth">
                        <UInput ref="dateInput" name="date"  autocomplete="off" @click="focusBirthDate()"  size="lg" v-model="state.birthDate" type="text"  :placeholder="$t('register_birthdate_label')"/>
                        <calendar />
                    </UFormGroup>
...
function getAcademicLevels(){
    GqlAcademicLevels().then(response => {
        ...
        navigateTo('/authentications/register/parent/student');
    }, error => {
        loadingStore.hide();
        toast.add(
            {
...
const onKeydownDateinput = (event: any) => {
