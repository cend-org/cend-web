    ...
    <div class="w-full flex justify-center mt-6">
        <div class="p-2 w-full lg:w-[25rem] xl:w-[25rem] 2xl:w-[25rem] flex flex-col gap-2">
            <form action="" class="w-full flex flex-col gap-2 pt-3">
                <h1 class="text-center text-xl text-gray-600 poppins-bold py-2">{{ $t('parent_register_academic_level')
                    }}</h1>
...
async function onSubmit() {
    ...
    if (studentId) {
        GqlSetStudentAcademicLevelByParent({ academicLevelId: parseInt(selectionSingleStore.selectedItem.Id), studentId: parseInt(studentId) }).then(response => {
    ...
    function getAcademicCourses() {
        GqlAcademicCourses({ academicLevelId: parseInt(selectionSingleStore.selectedItem.Id) }).then(response => {
            selectionMultipleStore.list = [];
            selectionMultipleStore.list = response.AcademicCourses as Array<never>;
            navigateTo("/authentications/register/parent/academic-course");
