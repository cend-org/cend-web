...
const onSubmit = handleSubmit(async (values) => {
    if (selectedSchoolSubject.value.length > 0) {
        loadingStore.show();
        try {
            await usr.setAcademicSubject(transform.ToArrayObject(selectedSchoolSubject.value, 'CourseId', "Id"), undefined , transform.ToArrayString(selectedSchoolSubject.value, "Name"));
            registration.next()
        } catch (e) {
            loadingStore.hide();
            toast({
