...
async function onSubmit() {
    ...
    GqlNewUserAcademicLevels({ academicLevelIds: selectedIds }).then(response => {
        getAcademicCourses(selectedIds);
    }, error => {
        loadingStore.hide();
        toast.add(
            {
    ...
    function getAcademicCourses(levels: Array<any>) {
        GqlMultipleLevelAcademicCourses({ academicLevelId: levels }).then(response => {
            selectionMultipleStore.clear();
            selectionMultipleStore.list = response.MultipleLevelAcademicCourses as Array<never>;
            navigateTo("/authentications/register/teacher/academic-course");
