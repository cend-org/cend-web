...
<Popover>
    <PopoverTrigger as-child>
      <Button type="button" class="flex flex-row items-center justify-start h-12 bg-white text-foreground shadow-none border w-full hover:bg-transparent">
        <CalendarIcon class="mr-2 h-4 w-4" />
        {{ placeholder ? df.format(placeholder.toDate(getLocalTimeZone())) : "Votre date de naissance" }}
      </Button>
    </PopoverTrigger>
    <PopoverContent class="w-auto p-0">
      <CalendarRoot
